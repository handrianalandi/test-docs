...
termination_reason_page_response = TerminationReasonPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
termination_reason_page_response = TerminationReasonPageResponse.from_json(json_str)

# Convert to JSON
print(termination_reason_page_response.to_json())
```
...
data_dict = termination_reason_page_response.to_dict()
termination_reason_page_response = TerminationReasonPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(termination_reason_page_response.total_pages)
# Update properties
termination_reason_page_response.total_pages = new_value
```
