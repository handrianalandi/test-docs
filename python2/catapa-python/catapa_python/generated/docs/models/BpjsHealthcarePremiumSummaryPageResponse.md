...
bpjs_healthcare_premium_summary_page_response = BpjsHealthcarePremiumSummaryPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
bpjs_healthcare_premium_summary_page_response = (
    BpjsHealthcarePremiumSummaryPageResponse.from_json(json_str)
)

# Convert to JSON
print(bpjs_healthcare_premium_summary_page_response.to_json())
```
...
data_dict = bpjs_healthcare_premium_summary_page_response.to_dict()
bpjs_healthcare_premium_summary_page_response = (
    BpjsHealthcarePremiumSummaryPageResponse.from_dict(data_dict)
)
```
...
```python
# Access properties
print(bpjs_healthcare_premium_summary_page_response.total_pages)
# Update properties
bpjs_healthcare_premium_summary_page_response.total_pages = new_value
```
