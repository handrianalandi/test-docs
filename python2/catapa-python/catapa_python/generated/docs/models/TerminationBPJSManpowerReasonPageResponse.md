...
termination_bpjs_manpower_reason_page_response = TerminationBPJSManpowerReasonPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
termination_bpjs_manpower_reason_page_response = (
    TerminationBPJSManpowerReasonPageResponse.from_json(json_str)
)

# Convert to JSON
print(termination_bpjs_manpower_reason_page_response.to_json())
```
...
data_dict = termination_bpjs_manpower_reason_page_response.to_dict()
termination_bpjs_manpower_reason_page_response = (
    TerminationBPJSManpowerReasonPageResponse.from_dict(data_dict)
)
```
...
```python
# Access properties
print(termination_bpjs_manpower_reason_page_response.total_pages)
# Update properties
termination_bpjs_manpower_reason_page_response.total_pages = new_value
```
