...
user_page_response = UserPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
    ...
    size=56,
    number=56,
    content=[],  # List[[UserResponse](UserResponse.md)]
)
```
...
user_page_response = UserPageResponse.from_json(json_str)

# Convert to JSON
print(user_page_response.to_json())
```
...
data_dict = user_page_response.to_dict()
user_page_response = UserPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(user_page_response.total_pages)
# Update properties
user_page_response.total_pages = new_value
```
