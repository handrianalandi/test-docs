...
employee_paygroup_page_response = EmployeePaygroupPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
employee_paygroup_page_response = EmployeePaygroupPageResponse.from_json(json_str)

# Convert to JSON
print(employee_paygroup_page_response.to_json())
```
...
data_dict = employee_paygroup_page_response.to_dict()
employee_paygroup_page_response = EmployeePaygroupPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(employee_paygroup_page_response.total_pages)
# Update properties
employee_paygroup_page_response.total_pages = new_value
```
