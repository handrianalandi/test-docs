...
bpjs_manpower_premium_summary_page_response = BpjsManpowerPremiumSummaryPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
bpjs_manpower_premium_summary_page_response = (
    BpjsManpowerPremiumSummaryPageResponse.from_json(json_str)
)

# Convert to JSON
print(bpjs_manpower_premium_summary_page_response.to_json())
```
...
data_dict = bpjs_manpower_premium_summary_page_response.to_dict()
bpjs_manpower_premium_summary_page_response = (
    BpjsManpowerPremiumSummaryPageResponse.from_dict(data_dict)
)
```
...
```python
# Access properties
print(bpjs_manpower_premium_summary_page_response.total_pages)
# Update properties
bpjs_manpower_premium_summary_page_response.total_pages = new_value
```
