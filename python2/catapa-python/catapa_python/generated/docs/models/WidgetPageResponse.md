...
widget_page_response = WidgetPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
    ...
    size=56,
    number=56,
    content=[],  # List[[WidgetResponse](WidgetResponse.md)]
)
```
...
widget_page_response = WidgetPageResponse.from_json(json_str)

# Convert to JSON
print(widget_page_response.to_json())
```
...
data_dict = widget_page_response.to_dict()
widget_page_response = WidgetPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(widget_page_response.total_pages)
# Update properties
widget_page_response.total_pages = new_value
```
