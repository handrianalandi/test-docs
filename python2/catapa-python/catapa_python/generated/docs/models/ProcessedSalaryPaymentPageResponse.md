...
processed_salary_payment_page_response = ProcessedSalaryPaymentPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
processed_salary_payment_page_response = ProcessedSalaryPaymentPageResponse.from_json(
    json_str
)

# Convert to JSON
print(processed_salary_payment_page_response.to_json())
```
...
data_dict = processed_salary_payment_page_response.to_dict()
processed_salary_payment_page_response = ProcessedSalaryPaymentPageResponse.from_dict(
    data_dict
)
```
...
```python
# Access properties
print(processed_salary_payment_page_response.total_pages)
# Update properties
processed_salary_payment_page_response.total_pages = new_value
```
