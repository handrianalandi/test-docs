...
role_page_response = RolePageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
    ...
    size=56,
    number=56,
    content=[],  # List[[RoleIdNameResponse](RoleIdNameResponse.md)]
)
```
...
role_page_response = RolePageResponse.from_json(json_str)

# Convert to JSON
print(role_page_response.to_json())
```
...
data_dict = role_page_response.to_dict()
role_page_response = RolePageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(role_page_response.total_pages)
# Update properties
role_page_response.total_pages = new_value
```
