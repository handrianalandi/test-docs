...
severance_plan_page_response = SeverancePlanPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
severance_plan_page_response = SeverancePlanPageResponse.from_json(json_str)

# Convert to JSON
print(severance_plan_page_response.to_json())
```
...
data_dict = severance_plan_page_response.to_dict()
severance_plan_page_response = SeverancePlanPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(severance_plan_page_response.total_pages)
# Update properties
severance_plan_page_response.total_pages = new_value
```
