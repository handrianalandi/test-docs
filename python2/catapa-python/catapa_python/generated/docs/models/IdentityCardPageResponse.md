...
identity_card_page_response = IdentityCardPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
identity_card_page_response = IdentityCardPageResponse.from_json(json_str)

# Convert to JSON
print(identity_card_page_response.to_json())
```
...
data_dict = identity_card_page_response.to_dict()
identity_card_page_response = IdentityCardPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(identity_card_page_response.total_pages)
# Update properties
identity_card_page_response.total_pages = new_value
```
