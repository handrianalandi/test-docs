...
employee_identity_card_page_response = EmployeeIdentityCardPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
employee_identity_card_page_response = EmployeeIdentityCardPageResponse.from_json(
    json_str
)

# Convert to JSON
print(employee_identity_card_page_response.to_json())
```
...
data_dict = employee_identity_card_page_response.to_dict()
employee_identity_card_page_response = EmployeeIdentityCardPageResponse.from_dict(
    data_dict
)
```
...
```python
# Access properties
print(employee_identity_card_page_response.total_pages)
# Update properties
employee_identity_card_page_response.total_pages = new_value
```
