...
salary_item_page_response = SalaryItemPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
salary_item_page_response = SalaryItemPageResponse.from_json(json_str)

# Convert to JSON
print(salary_item_page_response.to_json())
```
...
data_dict = salary_item_page_response.to_dict()
salary_item_page_response = SalaryItemPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(salary_item_page_response.total_pages)
# Update properties
salary_item_page_response.total_pages = new_value
```
