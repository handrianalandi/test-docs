...
payslip_additional_note_page_response = PayslipAdditionalNotePageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
payslip_additional_note_page_response = PayslipAdditionalNotePageResponse.from_json(
    json_str
)

# Convert to JSON
print(payslip_additional_note_page_response.to_json())
```
...
data_dict = payslip_additional_note_page_response.to_dict()
payslip_additional_note_page_response = PayslipAdditionalNotePageResponse.from_dict(
    data_dict
)
```
...
```python
# Access properties
print(payslip_additional_note_page_response.total_pages)
# Update properties
payslip_additional_note_page_response.total_pages = new_value
```
