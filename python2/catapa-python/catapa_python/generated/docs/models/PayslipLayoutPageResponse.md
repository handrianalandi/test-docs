...
payslip_layout_page_response = PayslipLayoutPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
payslip_layout_page_response = PayslipLayoutPageResponse.from_json(json_str)

# Convert to JSON
print(payslip_layout_page_response.to_json())
```
...
data_dict = payslip_layout_page_response.to_dict()
payslip_layout_page_response = PayslipLayoutPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(payslip_layout_page_response.total_pages)
# Update properties
payslip_layout_page_response.total_pages = new_value
```
