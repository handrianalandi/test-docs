...
sub_location_page_response = SubLocationPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
sub_location_page_response = SubLocationPageResponse.from_json(json_str)

# Convert to JSON
print(sub_location_page_response.to_json())
```
...
data_dict = sub_location_page_response.to_dict()
sub_location_page_response = SubLocationPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(sub_location_page_response.total_pages)
# Update properties
sub_location_page_response.total_pages = new_value
```
