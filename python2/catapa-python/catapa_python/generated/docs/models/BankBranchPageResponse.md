...
bank_branch_page_response = BankBranchPageResponse(
    total_pages=56,
    last=True,
    total_elements=56,
...
bank_branch_page_response = BankBranchPageResponse.from_json(json_str)

# Convert to JSON
print(bank_branch_page_response.to_json())
```
...
data_dict = bank_branch_page_response.to_dict()
bank_branch_page_response = BankBranchPageResponse.from_dict(data_dict)
```
...
```python
# Access properties
print(bank_branch_page_response.total_pages)
# Update properties
bank_branch_page_response.total_pages = new_value
```
